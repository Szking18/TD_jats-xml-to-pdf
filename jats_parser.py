...
def _parse_article_meta(am):
    ...
    for ptype in ['epub', 'ppub', 'collection']:
        ...
        if pd is not None:
            day = _find(pd, 'day')
            month = _find(pd, 'month')
            year = _find(pd, 'year')
            info[f'pub_date_{ptype}'] = {
                'day': _get_text(day) if day is not None else '',
                'month': _get_text(month) if month is not None else '',
                'year': _get_text(year) if year is not None else '',
            }
...
def _parse_date(elem):
    ...
    # 格式化日期字符串
    if parts.get('year'):
        date_str = f"{parts.get('year', '')}-{parts.get('month', '')}-{parts.get('day', '')}"
    else:
        date_str = ''
...
def get_history_text(doc):
    ...
    for entry in history:
        ...
        if dtype == 'received':
            lines.append(f"Received: {entry.get('formatted', '')}")
        elif dtype == 'rev-recd':
            lines.append(f"Revised: {entry.get('formatted', '')}")
        elif dtype == 'accepted':
            lines.append(f"Accepted: {entry.get('formatted', '')}")
    return lines
